...
| http://127.0.0.1:3000/view | ライブ表示（リスト形式・最新30件） |
...
|-----|------|
| `<BASE_URL>/` | コメント投稿フォーム |
| `<BASE_URL>/view` | ライブ表示。投稿されたコメントがリスト先頭に追加されます（最新30件を保持） |
| `<BASE_URL>/history` | コメント一覧。過去1時間かつ最大50件を新しい順に表示 |

### 基本的な使い方

1. **投稿**: `<BASE_URL>/` を開き、コメントと文字色を入力して「送信」
2. **ライブ表示**: 別タブや別端末で `<BASE_URL>/view` を開いておくと、新着コメントが自動でリストに追加されます
