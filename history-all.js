'use strict'

const statusEl = document.getElementById('status')
const listEl = document.getElementById('comment-list')
const refreshBtn = document.getElementById('btn-refresh')
const exportCsvBtn = document.getElementById('btn-export-csv')

/** @type {Array<{ id?: number, text?: string, color?: string, createdAt?: number }>} */
let loadedComments = []

/**
 * ステータス表示を更新する。
 *
 * @param {string} message 表示文言。
 */
const setStatus = (message) => {
  if (statusEl) {
    statusEl.textContent = message
  }
}

/**
 * CSV エクスポートボタンの有効状態を更新する。
 */
const updateExportButtonState = () => {
  if (exportCsvBtn) {
    exportCsvBtn.disabled = loadedComments.length === 0
  }
}

/**
 * epoch ms をローカル時刻文字列に変換する。
 *
 * @param {number} createdAt 投稿時刻（epoch ms）。
 * @returns {string}
 */
const formatTime = (createdAt) => {
  if (typeof createdAt !== 'number' || Number.isNaN(createdAt)) {
    return '--:--:--'
  }
  return new Date(createdAt).toLocaleString('ja-JP', {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
    month: '2-digit',
    day: '2-digit',
  })
}

/**
 * CSV 用にフィールドをエスケープする。
 *
 * @param {string} value 元の文字列。
 * @returns {string}
 */
const escapeCsvField = (value) => {
  const text = String(value)
  if (/[",\r\n]/.test(text)) {
    return `"${text.replace(/"/g, '""')}"`
  }
  return text
}

/**
 * ダウンロード用ファイル名を生成する。
 *
 * @returns {string}
 */
const buildCsvFilename = () => {
  const now = new Date()
  const pad = (num) => String(num).padStart(2, '0')
  const stamp = [
    now.getFullYear(),
    pad(now.getMonth() + 1),
    pad(now.getDate()),
    '-',
    pad(now.getHours()),
    pad(now.getMinutes()),
    pad(now.getSeconds()),
  ].join('')
  return `comments-${stamp}.csv`
}

/**
 * コメント一覧を CSV としてダウンロードする。
 *
 * @param {Array<{ id?: number, text?: string, color?: string, createdAt?: number }>} comments コメント配列。
 */
const exportCommentsCsv = (comments) => {
  if (!Array.isArray(comments) || comments.length === 0) {
    return
  }

  const header = ['id', 'createdAt', 'text', 'color']
  const rows = comments.map((comment) => {
    const createdAt =
      typeof comment.createdAt === 'number' && !Number.isNaN(comment.createdAt)
        ? new Date(comment.createdAt).toISOString()
        : ''
    return [
      escapeCsvField(comment.id ?? ''),
      escapeCsvField(createdAt),
      escapeCsvField(typeof comment.text === 'string' ? comment.text : ''),
      escapeCsvField(typeof comment.color === 'string' ? comment.color : ''),
    ].join(',')
  })

  const csv = `\uFEFF${[header.join(','), ...rows].join('\r\n')}`
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = buildCsvFilename()
  link.click()
  URL.revokeObjectURL(url)
}

/**
 * コメント 1 件の DOM を生成する。
 *
 * @param {{ id?: number, text?: string, color?: string, createdAt?: number }} comment コメント。
 * @returns {HTMLLIElement}
 */
const renderCommentItem = (comment) => {
  const item = document.createElement('li')
  item.className = 'comment-item'

  const timeEl = document.createElement('div')
  timeEl.className = 'comment-time'
  timeEl.textContent = formatTime(comment.createdAt)

  const swatch = document.createElement('div')
  swatch.className = 'color-swatch'
  swatch.style.backgroundColor =
    typeof comment.color === 'string' && comment.color.trim() !== '' ? comment.color : '#ffffff'
  swatch.title = swatch.style.backgroundColor

  const textEl = document.createElement('p')
  textEl.className = 'comment-text'
  textEl.textContent = typeof comment.text === 'string' ? comment.text : ''
  textEl.style.color = swatch.style.backgroundColor

  item.append(timeEl, swatch, textEl)
  return item
}

/**
 * 一覧を描画する。
 *
 * @param {Array<{ id?: number, text?: string, color?: string, createdAt?: number }>} comments コメント配列。
 */
const renderComments = (comments) => {
  loadedComments = Array.isArray(comments) ? comments : []
  updateExportButtonState()

  if (!listEl) {
    return
  }
  listEl.replaceChildren()
  if (loadedComments.length === 0) {
    const empty = document.createElement('li')
    empty.className = 'empty'
    empty.textContent = '表示できるコメントはありません'
    listEl.append(empty)
    setStatus('0 件')
    return
  }
  for (const comment of loadedComments) {
    listEl.append(renderCommentItem(comment))
  }
  setStatus(`${loadedComments.length} 件`)
}

/**
 * 全件履歴 API からコメント一覧を取得して描画する。
 *
 * @returns {Promise<void>}
 */
const loadHistory = async () => {
  if (refreshBtn) {
    refreshBtn.disabled = true
  }
  if (exportCsvBtn) {
    exportCsvBtn.disabled = true
  }
  setStatus('読み込み中…')
  try {
    const res = await fetch('/api/comments/history/all')
    let data = {}
    try {
      data = await res.json()
    } catch {
      /* 空ボディ等 */
    }
    if (!res.ok) {
      loadedComments = []
      updateExportButtonState()
      setStatus(typeof data.error === 'string' ? data.error : `エラー (${res.status})`)
      return
    }
    const comments = Array.isArray(data.comments) ? data.comments : []
    renderComments(comments)
  } catch (error) {
    loadedComments = []
    updateExportButtonState()
    setStatus('読み込みに失敗しました')
    console.error('loadHistory failed', error)
  } finally {
    if (refreshBtn) {
      refreshBtn.disabled = false
    }
  }
}

if (refreshBtn) {
  refreshBtn.addEventListener('click', () => {
    void loadHistory()
  })
}

if (exportCsvBtn) {
  exportCsvBtn.addEventListener('click', () => {
    exportCommentsCsv(loadedComments)
  })
}

void loadHistory()
